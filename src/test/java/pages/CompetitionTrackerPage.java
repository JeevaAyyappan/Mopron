...
public class CompetitionTrackerPage extends ElementActions{

	
	public CompetitionTrackerPage(){
	...
	@FindBy(xpath="//div[@class='owl-stage']//div/a[contains(.,'Competition Tracker')]")
	WebElement compTrackHeader;
	
	public boolean isDisplayCompTrackerPage()throws Exception{
		boolean status = false;
		try{	
			Wait.waitUntilElementPresent(compTrackHeader);
			highlightElement(Basetest.driver, compTrackHeader);
			getURL();
			accessCookies();
			if(compTrackHeader.isDisplayed()){
				return status = true;
			}
		}catch(Exception e){
			e.printStackTrace();
	...
	public static void accessCookies()throws Exception{
